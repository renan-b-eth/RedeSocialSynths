...
<head>
    ...
    <link href="css/style/styleMyFriends.css" rel="stylesheet" type="text/css"/>
    ...
    include_once ('./checkSession.php');
        ...
        <div id="container" style="width:1100px;">
            <div id="insurerLeft">
                <div id="insurerDados">
                    ...
                    <div id="fraseProfile">
                        ...
                        <img src="imagens/icones/IconesMeuPerfil/editarMeuPerfil.png" id="editProfile" width="100%" height="100%">
                    </div>
                </div>
           ...
           <div id="insurerRight" style=" width: 700px;">
            ...
            $sql = ("select * from tbAmigos where (idUser='$idUser' or idAmigo='$idUser') and status=3");  // faz o select das postagem da última para a primeira
            ...
            while ($linha = mysqli_fetch_array($result)) {  // pega todos os dados da tabela e coloca em um array    
                if( $linha['idUser'] != $idUser) {
                    ...
                    $idAmigo = $linha['idAmigo']; 
                }
                
                $sql2 = ("select * from tbUser where idUser='$idAmigo'");  // faz o select das postagem da última para a primeira
                $result2 = mysqli_query($conn, $sql2);
                $linha2 = mysqli_fetch_array($result2);
                $imgAmigo = $linha2['photoUser'];
                $nomeAmigo = $linha2['nameUser']." ".$linha2['lastNameUser'];
                // pega todas as datas e formata 
                echo '<div class="caixaAmigo">
                <div class="sup">
                <a href="./profile.php?nicknameUser='.$linha2['nicknameUser'].'"><div class="fotoAmigo"><img src="'.$imgAmigo.'" width="100%"></div>
                <div class="nomeAmigo"><b>'.$nomeAmigo.'</b></div></a>
                </div>
                <div class="inf">
                    <form name="teste" method="post" action="deleteFriend.php">
                        <input type="text" value="'.$idAmigo.'" style="display: none;" name="aux">
                        <input class="botaoDesfazer" type="submit" value="X" name="X">
                    </form>
                </div>
                </div>';
